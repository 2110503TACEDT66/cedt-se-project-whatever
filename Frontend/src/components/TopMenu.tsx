...
export default async function TopMenu() {
  ...
  return (
    <div
      className={`h-16 bg-white flex flex-row fixed top-0 left-0 right-0 border-b-[1px] border-solid ${
      ...
      <TopMenuItem
        title={
          session?.user.role === 'receptionist' ? 'Bookings' : 'My Booking'
        }
        imgSrc={
          session?.user.role === 'receptionist'
            ? '/img/bookingIcon2.png'
            : '/img/bookingIcon.png'
        }
        pageRef="/mybookings"
      />
